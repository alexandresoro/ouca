...
const AutocompleteMultipleWithSelection = <T extends object,>(
          ...
          {value.length > 0 && (
            <div className="flex flex-col gap-3 pb-4">
              {value.map((selectedValue) => (
                <div key={selectedValue[key] as Key} className="flex gap-4">
                  <input
                    type="checkbox"
                    className="checkbox checkbox-primary !animate-none"
                    checked
                    onChange={() => handleDeselectionFromList(selectedValue)}
                  />
                  <span className="cursor-default">{renderValue(selectedValue)}</span>
                </div>
              ))}
            </div>
          )}
...
export default forwardRef(AutocompleteMultipleWithSelection);
